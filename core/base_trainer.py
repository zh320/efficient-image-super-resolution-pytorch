...
from datasets import get_loader, get_test_loader, get_val_loader
...
class BaseTrainer:
    def __init__(self, config):
        ...
        if config.is_testing:
            ...
            self.val_loaders = get_val_loader(config)
        else:
            # Tensorboard monitor
        ...
        if not config.is_testing and not config.benchmark:
    ...
    def run(self, config):
        ...
        for cur_epoch in range(start_epoch, config.total_epoch):
            ...
            if cur_epoch - self.best_epoch > config.early_stop_epoch:
                break
    ...
    def load_ckpt(self, config):
        if config.load_ckpt and os.path.isfile(config.load_ckpt_path):
            ...
            if not config.is_testing and not config.benchmark and config.resume_training:
